...
export default function SettingsPage() {
  ...
  useEffect(() => {
    // Lấy ID không gian riêng của người dùng đã đăng nhập
            ...
            <div className="bg-white rounded-[50px] p-10 max-w-md w-full text-center shadow-2xl relative border border-white/20">
                {paymentStatus === "pending" ? (
                    <div className="animate-in fade-in zoom-in duration-300">
                        <button onClick={() => setShowQR(false)} className="absolute top-8 right-8 text-slate-300 hover:text-red-500 transition-colors"><X size={32} /></button>
                        <div className="mb-6">
                            ...
                            <p className="text-xs font-bold text-slate-400 mt-1 uppercase">Gói {paymentInfo.plan} • {paymentInfo.amount.toLocaleString()}đ</p>
                        </div>
                        <div className="bg-white p-6 rounded-[40px] mb-8 border-2 border-slate-100 shadow-sm">
                            <img src={paymentInfo.qr} className="w-full rounded-3xl" alt="VietQR" />
                        </div>
                        <div className="space-y-4">
                            <div className="bg-blue-600 p-5 rounded-3xl text-white cursor-pointer active:scale-95 transition-all shadow-xl" onClick={() => {navigator.clipboard.writeText(paymentInfo.memo); alert("Đã copy nội dung!");}}>
                            ...
                            <div className="flex items-center justify-center gap-2 text-[11px] text-blue-600 font-black uppercase animate-pulse mt-4"><Loader2 size={14} className="animate-spin" /> Đang chờ tiền về hệ thống...</div>
                        </div>
                    </div>
                ) : (
                    <div className="py-6 animate-in zoom-in-50 duration-500 text-black">
                        ...
                        <p className="text-sm text-slate-500 font-bold mb-10 px-4">Chúc mừng bạn! Gói {paymentInfo.plan} đã được kích hoạt thành công.</p>
...
function AccountTab() {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                ...
                setUser(res.data);
            } catch (e) { console.log("Guest mode"); } finally { setLoading(false); }
        };
        fetchProfile();
    }, [API_URL]);
    ...
    if (loading) return <div className="p-10 text-center animate-pulse font-black text-slate-300">XÁC MINH DANH TÍNH...</div>;
        ...
        <div className="space-y-10 animate-in fade-in">
            ...
            {!user ? (
                <div className="bg-slate-50 p-10 rounded-[35px] text-center border-2 border-dashed border-slate-200">
                    <p className="text-slate-400 font-black uppercase mb-4 text-sm">Bạn đang truy cập với tư cách Khách</p>
                    <button onClick={handleGoogleLogin} className="bg-blue-600 text-white px-10 py-4 rounded-2xl font-black shadow-lg">ĐĂNG NHẬP NGAY</button>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-3">
                        <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Họ và tên</label>
                        <input className="w-full p-5 bg-slate-50 rounded-[20px] outline-none font-bold text-black border border-transparent" value={user.name || "N/A"} readOnly />
                    </div>
                    <div className="space-y-3">
            ...
            <div className="pt-10 border-t border-slate-100">
                <h3 className="font-black text-lg mb-6 flex items-center gap-2 italic text-slate-900"><Globe size={20} className="text-blue-600" /> Kết nối xác thực</h3>
                <button onClick={handleGoogleLogin} className="flex items-center justify-between p-6 bg-white border-2 border-slate-100 rounded-[30px] hover:border-blue-500 transition-all group w-full md:w-2/3 font-black">
                    <div className="flex items-center gap-4"><img src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/list/google.svg" className="w-6 h-6" alt="G" /> {user ? "Đã kết nối Google" : "Tiếp tục với Google"}</div>
                    <span className={`text-[10px] uppercase px-4 py-1 rounded-full ${user ? 'bg-green-50 text-green-500' : 'bg-slate-50 text-slate-400'}`}>{user ? 'Đã bảo mật' : 'Xác thực ngay'}</span>
                </button>
            </div>
        </div>
    )
...
function BillingTab({ onUpgrade }: any) {
    ...
    return (
        <div className="space-y-10 animate-in fade-in">
            <h2 className="text-2xl font-black italic uppercase text-slate-900">Gói dịch vụ</h2>
...
function SecurityTab() { return <div className="p-10 text-center"><Lock size={48} className="mx-auto text-slate-200 mb-4" /><p className="font-black text-slate-400 uppercase italic">Bảo mật đa tầng đang được kích hoạt...</p></div> }
function VoucherTab() { return <div className="p-10 text-center"><Gift size={48} className="mx-auto text-slate-200 mb-4" /><p className="font-black text-slate-400 uppercase italic">Bạn chưa có mã giảm giá nào.</p></div> }
function GuideTab() { return <div className="p-10 text-center"><BookOpen size={48} className="mx-auto text-slate-200 mb-4" /><p className="font-black text-slate-400 uppercase italic">Hướng dẫn đang được AI biên soạn...</p></div> }
